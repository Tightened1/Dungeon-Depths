...
const ALL_RELICS=[
  ...
  {id:'gilded_coin',name:'Gilded Coin',icon:'🪙',type:'pure',
   buffs:['+50 gold','Items sell for 20% more'],debuffs:[],
   flavor:'Fortune favours the bold.',
   apply:(p)=>{p.gold+=50;p.sellBonus=(p.sellBonus||0)+0.2}},
  ...
  {id:'sage_eye',name:'Sage Eye',icon:'👁',type:'pure',
   buffs:['Reveal entire floor (permanent)'],debuffs:[],
   flavor:'See past every shadow.',
   apply:(p)=>{p.allSight=true}},
  {id:'bounty_mark',name:'Bounty Mark',icon:'🎯',type:'pure',
   buffs:['+6 gold per kill'],debuffs:[],
   flavor:'Every enemy is worth something.',
   apply:(p)=>{p.bountyGold=(p.bountyGold||0)+6}},
  {id:'mana_shard',name:'Mana Shard',icon:'🔮',type:'pure',
   buffs:['Spell fail chance reduced by 10%'],debuffs:[],
   flavor:'Crystallised arcane focus.',
   apply:(p)=>{p.spellResist=(p.spellResist||0)+10}},
  ...
  {id:'cursed_blade',name:'Cursed Blade',icon:'🔪',type:'balanced',
   buffs:['+10 ATK on every attack'],debuffs:['-2 HP on every attack (self)'],
   flavor:'It thirsts — for your blood too.',
   apply:(p)=>{p.cursedBlade=true}},
  ...
  {id:'eclipse_ring',name:'Eclipse Relic',icon:'🌘',type:'balanced',
   buffs:['+5 ATK','+3 DEF'],debuffs:['No gold drops from enemies'],
   flavor:'Power that costs your fortune.',
   apply:(p)=>{p.atk+=5;p.def+=3;p.noGoldDrops=true}},
  {id:'warlord_band',name:'Warlord Band',icon:'🏴',type:'balanced',
   buffs:['Boss damage +15'],debuffs:['-5 DEF vs non-boss enemies'],
   flavor:'Forged for the hunt of kings.',
   apply:(p)=>{p.smiteBonus=(p.smiteBonus||0)+15;p.antiSmite=(p.antiSmite||0)+5}},
  ...
  {id:'betrayer_ring',name:'Betrayer Ring',icon:'🔴',type:'cursed',
   buffs:['+15 ATK'],debuffs:['Take double damage from all sources'],
   flavor:'Strength for fragility.',
   apply:(p)=>{p.atk+=15;p.dmgMult=(p.dmgMult||1)*2}},
